...
class FaceGraphic(
    ...
    override fun draw(canvas: Canvas) {
        ...
        val x = translateX(face.boundingBox.centerX().toFloat(), image)
        val y = translateY(face.boundingBox.centerY().toFloat(), image)
        ...
        val xOffset = scaleX(face.boundingBox.width() / 2.0f, image)
        val yOffset = scaleY(face.boundingBox.height() / 2.0f, image)
        val left = x - xOffset
        val top = y - yOffset
        val right = x + xOffset
    ...
    private fun drawLandmarkPosition(canvas: Canvas, face: FirebaseVisionFace, landmarkID: Int) {
        ...
        landmark?.let {
            ...
            canvas.drawCircle(
                translateX(point.x, image),
                translateY(point.y, image),
                10f, idPaint
            )
        }
    }
    ...
    private fun drawBitmapOverLandmarkPosition(
        ...
        overlayBitmap?.let {
            ...
            val left = (translateX(point.x, image) - imageEdgeSizeBasedOnFaceSize).toInt()
            val top = (translateY(point.y, image) - imageEdgeSizeBasedOnFaceSize).toInt()
            val right = (translateX(point.x, image) + imageEdgeSizeBasedOnFaceSize).toInt()
            val bottom = (translateY(point.y, image) + imageEdgeSizeBasedOnFaceSize).toInt()
